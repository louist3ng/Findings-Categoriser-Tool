...
5. **Layer 5 — LLM Fallback** (optional): Uses Claude or Gemini API with enriched context: vulnerability rule name, severity, CVSS score, CWE, OWASP Mobile, MASVS, description, obfuscation flag, sibling file paths (other files flagged under the same rule), and **per-file API/behaviour profiles** extracted from the MobSF report (`android_api` and `behaviour` sections). The API profile reveals what a class actually does (e.g. HTTP connections, file I/O, clipboard access) even when class names are obfuscated. Provider is auto-detected from available API keys or set explicitly via `--llm-provider`.
