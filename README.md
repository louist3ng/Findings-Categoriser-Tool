...
5. **Layer 5 — LLM Fallback** (optional): Uses Claude or Gemini API with enriched context: vulnerability rule name, severity, CVSS score, CWE, OWASP Mobile, MASVS, description, obfuscation flag, and sibling file paths (other files flagged under the same rule). Sibling paths help the LLM identify package patterns even for obfuscated code. Provider is auto-detected from available API keys or set explicitly via `--llm-provider`.
